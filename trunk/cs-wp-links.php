<?php
defined( '_CS_WEBDIR_' ) or die( '' );

if (defined( '_CSLIB_WP_LINKS_')) return;

define( _CSLIB_WP_LINKS_, true );
...
class _CS_WP_LINKS {
	...
	function _CS_WP_LINKS( $options ) {
		...
		//if (is_callable(array('cs_theme','description')) ) {
			add_filter( 'cs_wp_head_desc', array($this, 'filter_desc'), 10, 1 );
		//} else {
	...
	function list_categories( $beforelist='',$afterlist='',
		...
		apply_filters( 'cs_webdir_categories', $out );
	...
	function list_bookmarks( $linkcat=array(), $beforelist='', $afterlist='',
		...
		apply_filters( 'cs_webdir_bookmarks', $out );
	...
	function submit_link( $label='', $url='' ) {
		...
		return apply_filters( 'cs_webdir_submiturl', $submit );
	}
}
 ...
 */
class _CS_WPCF7_LINKCAT {

	var $links;

	function _CS_WPCF7_LINKCAT( $linksobj ) {
